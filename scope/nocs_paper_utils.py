...
def compute_3d_iou(bbox_3d_1, bbox_3d_2, handle_visibility, class_name_1, class_name_2):
    ...
    if class_name_1 in ['luggage_IDX']:
        box_symmetry = True
    ...
    if symmetry_flag:
        ...
        max_iou = 0
        # Flip combinations along x, y, z (0: no flip, 1: flip)
        for flip_x in [1, -1]:
            for flip_y in [1, -1]:
                for flip_z in [1, -1]:
                    # Construct diagonal flip matrix
                    flip_matrix = np.diag([flip_x, flip_y, flip_z])
                    bbox_center = np.mean(bbox_3d_1, -1, keepdims=True)
                    flipped_bbox = flip_matrix @ (bbox_3d_1 - bbox_center) + bbox_center
                    max_iou = max(max_iou, asymmetric_3d_iou(flipped_bbox, bbox_3d_2))
        return max_iou
...
def compute_degree_cm_mAP(final_results, synset_names, log_dir, degree_thresholds=[360], shift_thresholds=[100], iou_3d_thresholds=[0.1], iou_pose_thres=0.1, use_matches_for_pose=False):
    ...
    print(iou_3d_aps.shape) 
    print(iou_3d_aps)
    print("baasdfkasdfk")
    print(iou_3d_aps[1:-1, :])
...
def draw_3d_bbox(image, save_dir, data_name, image_id, camera_intrinsics, gt_RTs, gt_scales, pred_RTs, pred_scales):
    ...
    intrinsics = np.array([[camera_intrinsics['fx'], 0, camera_intrinsics['cx']], [0, camera_intrinsics['fy'], camera_intrinsics['cy']], [0, 0, 1]])
    ...
    for ind, RT in enumerate(gt_RTs):
        ...
        transformed_bbox_3d = transform_coordinates_3d(bbox_3d, RT)
        projected_bbox = calculate_2d_projections(transformed_bbox_3d, intrinsics)
        draw_image_bbox = draw(draw_image_bbox, projected_bbox, projected_axes, (0, 0, 255))
...
def draw_3d_bbox_est(image, save_dir, data_name, image_id, camera_intrinsics, pred_RTs, pred_scales):
